...
export default async function PropertiesPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    <SectionContainer className="max-w-full space-y-6 overflow-x-hidden py-10 sm:space-y-8 sm:py-14 lg:py-20">
      <h1 className="max-w-full text-3xl font-semibold leading-tight sm:text-4xl">Property Listing</h1>
      <PropertyFilters
        defaults={{
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <p className="min-w-0 text-sm text-[#616977]">Showing <span className="font-semibold text-[#111111]">{properties.length}</span> {properties.length === 1 ? 'listing' : 'listings'}</p>
      </div>
